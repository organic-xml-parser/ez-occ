...
from pythonoccutils.gears.gear_generator import GearSpec, GearPairSpec, InvoluteGearFactory
from pythonoccutils.occutils_python import WireSketcher, InterrogateUtils
from pythonoccutils.part_cache import PartCache, CacheToken, FileBasedPartCache, InMemoryPartCache, DefaultCacheToken
from pythonoccutils.part_manager import PartFactory, Part, NoOpPartCache
from pythonoccutils.stock_parts import StockParts, StepperDriver

logger = logging.getLogger(__name__)


def make_phone() -> Part:
    return PartFactory.box(10.6, 70, 139, z_min_face_name="bottom") \
...
if __name__ == '__main__':
    ...
    cache = FileBasedPartCache("/wsp/output/cache")
    ...
    def make_document():
        ...
        return stepper.driver(StepperDriver).get_rendered_part()\
            .add(
                gear_top.annotate("color", "green"),
                top_interface,
                gear_stepper.annotate("color", "green"),
                gear_idler.name("idler").annotate("color", "green"),
                gearbox_ymin,
                gearbox_ymax.tr.rz(math.radians(180)).align().by("xmidymidzmid", gearbox_ymin).tr.mv(dx=150))

    #make_document_token = cache.create_token(inspect.getsource(make_document))
    make_document_token = DefaultCacheToken.with_uuid("0e894dcd-a696-5248-9cb0-1aa536abc062", cache)

    logger.info("Starting visualization")
    cache.ensure_exists(make_document_token, lambda : make_document().with_cache_token(make_document_token)).preview()
