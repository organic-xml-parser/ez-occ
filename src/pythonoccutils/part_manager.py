...
from pythonoccutils.subshape_mapping import SubshapeMap, T_MKS, AnnotatedShape
...
class Part:
    ...
    def do_on(self, *names: str, consumer: typing.Callable[[Part], Part] = None) -> Part:
        if len(names) == 0:
        ...
        if len(names) == 1:
            name = names[0]
            subpart = self.single_subpart(name)
            modified_subpart = consumer(subpart)

            return self.remove(subpart).insert(modified_subpart.name(name))
        else:
            return self.do_on(names[0], consumer=lambda p: p.do_on(*names[1:], consumer=consumer))
    ...
    def compound_subpart(self, name: str) -> Part:
        """
        ...
        subshape = PartFactory(self._cache_token.get_cache())\
            .compound(*[Part.of_shape(s.shape).name(name) for s in subshapes]).shape
    ...
    def sp(self, *names) -> Part:
        return self.single_subpart(*names)

    def single_subpart(self, *names: str) -> Part:
        """
        ...
        subshape = self._subshapes.get_single(name)
...
class PartSave:
    ...
    def load_ocaf(path: str, part_cache: PartCache) -> Part:
        ...
        from pythonoccutils.part_cache import DefaultCacheToken
    ...
    def _downcast_shape(shape: OCC.Core.TopoDS.TopoDS_Shape):
        if shape.ShapeType() == OCC.Core.TopAbs.TopAbs_SHAPE:
            ...
            return OCC.Core.TopoDS.topods_Compound(shape)
        elif shape.ShapeType() == OCC.Core.TopAbs.TopAbs_COMPSOLID:
            return OCC.Core.TopoDS.topods_CompSolid(shape)
        elif shape.ShapeType() == OCC.Core.TopAbs.TopAbs_SOLID:
            return OCC.Core.TopoDS.topods_Solid(shape)
        elif shape.ShapeType() == OCC.Core.TopAbs.TopAbs_SHELL:
            return OCC.Core.TopoDS.topods_Shell(shape)
        elif shape.ShapeType() == OCC.Core.TopAbs.TopAbs_FACE:
            return OCC.Core.TopoDS.topods_Face(shape)
            ...
            return OCC.Core.TopoDS.topods_Edge(shape)
        elif shape.ShapeType() == OCC.Core.TopAbs.TopAbs_VERTEX:
            return OCC.Core.TopoDS.topods_Vertex(shape)
        else:
            raise ValueError()
...
class PartBool:
    ...
    def union(self, *others: Part, glue: OCC.Core.BOPAlgo.BOPAlgo_GlueEnum = None) -> Part:
        ...
        if len(others) == 0:
            ...
            tools = [Part(CacheToken(),
                          self._part.subshapes.with_updated_root_shape(s)) for s in op.InterrogateUtils.traverse_direct_subshapes(self._part.shape)]

            return self._boolop(self._part.cache_token.mutated("boolop", "union", tools[0:1], tools[1:]),
                                fuse,
                                tools[0:1],
                                tools[1:])

        return self._boolop(self._part.cache_token.mutated("boolop", "union", [self._part], others), fuse, [self._part], [p for p in others])

    def cut(self, *others: Part) -> Part:
        if len(others) == 0:
            raise ValueError("No other parts specified")

        return self._boolop(self._part.cache_token.mutated("boolop", "cut", [self._part], others),
    ...
    def common(self, *others: Part) -> Part:
        ...
        return self._boolop(self._part.cache_token.mutated("boolop", "common", [self._part], others),
    ...
    def section(self, *others: Part):
        ...
        return self._boolop(self._part.cache_token.mutated("boolop", "section", [self._part], others),
...
class PartMirror:
    ...
    def x(self, union: bool = False, center_x: float = 0):
        result = PartTransformer(self._part)(
            lambda t: t.SetMirror(gp.gp_YOZ().Translated(gp.gp_Vec(center_x, 0, 0))))
    ...
    def y(self, union: bool = False, center_y: float = 0):
        result = PartTransformer(self._part)(
            lambda t: t.SetMirror(gp.gp_ZOX().Translated(gp.gp_Vec(0, center_y, 0))))
    ...
    def z(self, union: bool = False, center_z: float = 0):
        result = PartTransformer(self._part)(
            lambda t: t.SetMirror(gp.gp_XOY().Translated(gp.gp_Vec(0, 0, center_z))))
...
class PartFilleter:
    ...
    def _create_selector(selector: typing.Optional[typing.Union[
        ...
        if selector is None:
            ...
            return lambda s: s == selector.shape
    ...
    def fillet_edges(self,
        ...
        return self._part.perform_make_shape(self._part.cache_token.mutated("fillet", "fillet_edges", edges), mkf)
    ...
    def chamfer_edges(self,
        ...
        return self._part.perform_make_shape(self._part.cache_token.mutated("fillet", "chamfer_edges", edges), mkf)
    ...
    def chamfer_faces(self,
        ...
        return self._part.perform_make_shape(self._part.cache_token.mutated("fillet", "chamfer_faces", faces), mkf)
...
class PartLoft:
    ...
    def pipe(self,
        ...
        if not isinstance(spine_part, Part):
            raise ValueError("Spine should be of type Part")

        binorm_cache = None if bi_normal_mode is None else (bi_normal_mode.X(), bi_normal_mode.Y(), bi_normal_mode.Z())

        token = self._part.cache_token.mutated("loft", "pipe", spine_part, transition_mode, binorm_cache)
...
class PartRevol:
    ...
    def about(self,
        ...
        result = self._part.perform_make_shape(make_shape)
...
class PartFactory:
    ...
    def face(self,
        ...
        return Part(self._cache.create_token("part_factory", "face", mkf.Face()), mkf.Face())
    ...
    def thread(self,
        ...
        def _do():
            ...
            if separate_sections:
                ...
                half_turn = tri.loft.pipe(half_turn_helix, bi_normal_mode=gp.gp_DZ())
                half_turns = []
    ...
    def conical_helix(self,
        ...
        cone = gp.gp_Cone(gp.gp_Ax3(gp.gp_Origin(), gp.gp_DZ(), gp.gp_DX()), conical_angle, radius_1)
    ...
    def polygon(self,
        ...
        result = ws.close().get_wire_part(self._cache)
    ...
    def hex_lattice(self,
        ...
        if modifier_callback is None:
            modifier_callback = lambda r, c, unit_part: unit_part

        base_shape = self.polygon(hex_radius, 6).make.face()
        ...
        for row in range(0, rows):
            ...
            for c in range(0, cols):
                ...
                subpart = modifier_callback(row, c, subpart)
    ...
    def lattice(self, rows: int, cols: int, diag_a: bool=False, diag_b: bool=False) -> Part:
        ...
        return Part(self._cache.create_token("part_factory", "lattice", rows, cols, diag_a, diag_b),
                    op.BoolUtils.incremental_fuse(wires))
    ...
    def loft(self,
             wires_or_faces: typing.List[typing.Union[Part, OCC.Core.TopoDS.TopoDS_Wire, OCC.Core.TopoDS.TopoDS_Face]],
             is_solid: bool = True,
             is_ruled: bool = True,
             pres3d: bool = 1.0e-6,
             first_shape_name: str = None,
        ...
        return Part(self._cache.create_token(ts.Shape()), SubshapeMap.from_unattributed_shapes(ts.Shape(), named_subshapes))
    ...
    def square_centered(self,
        ...
        if fill_face:
            return ws.get_face_part(self._cache)
        else:
            return ws.get_wire_part(self._cache)
    ...
    def __init__(self, section, line: OCC.Core.gp.gp_Lin, picked_part: Part):
        ...
        def get_vert_param(vert):
            return OCC.Core.GeomAPI.GeomAPI_ProjectPointOnCurve(
                OCC.Core.BRep.BRep_Tool_Pnt(vert.shape), curve).Parameter(1)
...
class PartPick:
    ...
    def dir(self,
        ...
        ray = Part.of_shape(OCC.Core.BRepBuilderAPI.BRepBuilderAPI_MakeEdge(line).Edge())
